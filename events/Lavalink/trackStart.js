...
module.exports = async (client, player, track, payload) => {
  // Skip notifications for TTS tracks
  if (track && track.isTTS) {
    return;
  }

  // Cancel music inactivity timer when new track starts
  if (client.musicInactivityTimers && client.musicInactivityTimers.has(player.guild)) {
    const timerId = client.musicInactivityTimers.get(player.guild);
    clearTimeout(timerId);
    client.musicInactivityTimers.delete(player.guild);
  }
  ...
  try {
    ...
    if (guild) {
      ...
      if (voiceChannel && voiceChannel.type === 2) { // Voice channel type
        // Check if setStatus method exists before calling it
        if (typeof voiceChannel.setStatus === 'function') {
          const statusText = `<a:music_disc:1379440965353345057> Playing ${track.title}`;
          await voiceChannel.setStatus(statusText.slice(0, 500)); // Discord has a 500 character limit
        }
      }
    }
  } catch (error) {
  ...
  const collector = m.createMessageComponentCollector({
    ...
    time: Math.min(track?.duration ?? queue?.duration ?? 300000, 2147483647), // Cap at 32-bit max or 5 minutes
  });
