...
module.exports = async (client, oldState, newState) => {
    // Handle voice recording cleanup when recording user leaves
    if (client.voiceRecordings && client.voiceRecordings.has(newState.guild.id)) {
        const recording = client.voiceRecordings.get(newState.guild.id);
        
        // If the user who started recording left the voice channel
        if (oldState.member && oldState.member.id === recording.userId && !newState.channel) {
            await client.voiceRecordingHandler.cleanupRecording(newState.guild.id);
            
            // Notify in text channel
            const textChannel = newState.guild.channels.cache.get(recording.textChannelId);
            if (textChannel) {
                await textChannel.send({
                    embeds: [new EmbedBuilder()
                        .setColor('#ff9500')
                        .setDescription(`<:vc:1379473053708189809> | Recording stopped because the recorder left the voice channel.`)
                    ]
                });
            }
        }
